...
set(expected_ranks 3)
execute_process(
    COMMAND
        "${MPIEXEC_EXECUTABLE}" "${MPIEXEC_NUMPROC_FLAG}" ${expected_ranks}
        ${MPIEXEC_PREFLAGS}
        "${PROBE}"
        ${MPIEXEC_POSTFLAGS}
        "${MODE}"
...
string(
    REGEX MATCHALL
    "observed quality-metrics MPI_Abort on affected communicator"
    abort_markers
    "${probe_output}"
)
list(LENGTH abort_markers abort_count)
if(NOT abort_count EQUAL expected_ranks)
    message(
        FATAL_ERROR
        "expected exactly one affected-communicator abort marker per rank (${expected_ranks}); found ${abort_count}\n${probe_output}"
    )
endif()
...
if(diagnostic_offset EQUAL -1)
    message(
        FATAL_ERROR
        "missing quality-metrics failure diagnostic '${EXPECTED_DIAGNOSTIC}'\n${probe_output}"
    )
endif()
