...
void reset_graph_partition(kahip::modified::graph_access& graph, int rank) {
  for (auto node = kahip::modified::NodeID{0}; node < graph.number_of_nodes();
       ++node) {
...
[[nodiscard]] auto partition_vector(kahip::modified::graph_access& graph)
    -> std::vector<kahip::modified::PartitionID> {
  auto result = std::vector<kahip::modified::PartitionID>(
...
TEST_CASE("evolutionary driver owns an errors-return communicator") {
  ...
  {
    auto driver = kahip::modified::parallel_mh_async{caller};
...
TEST_CASE("evolutionary gossip owns payloads through exact P2P completion") {
  ...
  kahip::modified::random_functions::setSeed(127 + rank);
...
TEST_CASE(
  ...
  config.initial_partitioning_type =
      kahip::modified::INITIAL_PARTITIONING_BIPARTITION;
...
TEST_CASE("initial partition refinement rejects a missing block target") {
  ...
  CHECK_THROWS_AS(kahip::modified::ip_partition_accept_rule(
                      config, 2, 4, 4, kahip::modified::PartitionID{0},
                      kahip::modified::PartitionID{1}),
...
TEST_CASE("reusing an evolutionary driver resets first-run state") {
  ...
  config.ultra_fast_kaffpaE_interfacecall = false;
  ...
  auto metrics = kahip::modified::quality_metrics{};
