...
[[nodiscard]] auto expected_abort_state() noexcept -> bool {
  ...
  switch (selected) {
    ...
    case mode::unfinished_teardown:
      return duplications == 0 && sendrecvs == 0 && isends == 0 &&
...
int main(int argc, char** argv) {
  ...
  if (selected == mode::unfinished_teardown) {
    evolutionary_failure_probe::active = true;
    {
      auto unfinished_exchange = exchanger{communicator};
      static_cast<void>(unfinished_exchange);
    }
  }
